export default function FeaturedProject({ title, description, tags, link = '#', letter = 'D', category = '' }) {
  return (
    <div className="grid grid-cols-2 border-b border-line max-[960px]:grid-cols-1">
      ...
      <div className="cursor-hover bg-cream-2 border-r border-line aspect-square flex items-center justify-center overflow-hidden relative group max-[960px]:aspect-video max-[960px]:border-r-0">
        ...
        />
        <div className="font-playfair text-[200px] font-black text-cream-3 leading-none tracking-tighter italic transition-transform duration-600 group-hover:scale-112 group-hover:-rotate-4 max-[960px]:text-[120px]"
      ...
      <div className="p-15 flex flex-col justify-center max-[960px]:p-6 max-[960px]:py-10">
        <div className="font-mono text-[11px] text-ink-4 tracking-[2px] uppercase mb-5 flex items-center gap-2.5">
        ...
        <h3 className="font-playfair text-[clamp(36px,4vw,56px)] font-black tracking-tight leading-[1.05] mb-5">
          {title.split(' — ').map((part, i) =>
            i === 1 ? <em key={i} className="italic text-ink-3"> — {part}</em> : <span key={i}>{part}</span>
          )}
        </h3>
        <p className="text-base text-ink-2 leading-[1.75] mb-9 font-light max-w-[420px]">
          {description}
        </p>
        <a
          href={link}
