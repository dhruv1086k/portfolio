...
export default function PixelCanvas({ src, pixelSize }) {
  ...
  useEffect(() => {
      ...
      img.onload = () => {
        ...
        for (let r = 0; r < rows; r++) {
          for (let c = 0; c < cols; c++) {
            ...
            /* Unique noise offset per particle for organic variation */
            ...
            pixels.push({
              ...
              vx: 0,
              vy: 0,
              noiseOffset, // per-particle phase for organic shimmer
            });
          }
        }
    ...
    const startAnimation = (ctx, W, H, pSize) => {
      ...
      /* ── Subtle hover constants ── */
      const MOUSE_RADIUS = 110; // slightly wider soft field
      const MOUSE_RADIUS_SQ = MOUSE_RADIUS * MOUSE_RADIUS;
      const MOUSE_FORCE = 1.6; // very gentle nudge
      const NOISE_SPEED = 0.0018; // how fast the shimmer oscillates
      const NOISE_AMP = 0.55; // max extra velocity from shimmer
      ...
      const loop = (now) => {
        ...
        for (let i = 0, len = pixels.length; i < len; i++) {
          ...
          if (phaseRef.current === "assembling") {
            ...
            /* ── Phase: settled (spring + subtle hover) ─── */
          } else {
            if (mouse.active) {
              ...
              if (distSq < MOUSE_RADIUS_SQ && distSq > 0) {
                /* Smooth 0→1 falloff — particles close to cursor move more */
                const dist = Math.sqrt(distSq);
                const falloff = 1 - dist / MOUSE_RADIUS; // linear 1→0
                const smooth = falloff * falloff; // ease it

                /* Organic shimmer: each particle oscillates at its own phase */
                const shimmerAngle =
                  px.noiseOffset + elapsed * NOISE_SPEED * (1 + smooth * 2);
                const nx = Math.cos(shimmerAngle);
                const ny = Math.sin(shimmerAngle);

                /* Combine a tiny outward nudge with the organic shimmer */
                const nudge = MOUSE_FORCE * smooth;
                const shimmer = NOISE_AMP * smooth;

                px.vx += (dx / dist) * nudge + nx * shimmer;
                px.vy += (dy / dist) * nudge + ny * shimmer;
              }
            }
