    ...
    >
        ...
        <ScrollReveal>
          <div className="cursor-hover relative flex-1 mb-10 bg-cream-2 rounded-none overflow-hidden flex items-center justify-center group max-[960px]:min-h-[280px]">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 z-10" style={{ background: 'linear-gradient(to top, rgba(237,233,224,0.95) 60%, transparent)' }}>
              ...
              <div className="text-[13px] text-ink-3 mt-0.5 font-mono tracking-[1px]">
                {SITE_CONFIG.role}
              </div>
            </div>
          </div>
      ...
      <div className="px-16 pt-[100px] pb-20 flex flex-col justify-center gap-15 max-[960px]:px-6 max-[960px]:pt-15 max-[960px]:pb-15">
        ...
        <div>
          ...
          <ScrollReveal>
            <h2
              className="font-playfair font-black leading-[1.05] mb-7"
              style={{ fontSize: 'clamp(40px, 5vw, 64px)', letterSpacing: '-2.5px' }}
            >
              Passionate<br />about <em className="italic text-ink-3">craft.</em>
            </h2>
          </ScrollReveal>
          {SITE_CONFIG.bio.map((paragraph, i) => (
            <ScrollReveal key={i} delay={0.1 * (i + 1)}>
              <p
                className="text-[17px] text-ink-2 leading-[1.8] font-light max-w-[540px]"
        ...
        <ScrollReveal delay={0.3}>
          <div className="border-t border-line">
            {TIMELINE.map((item) => (
              <TimelineItem key={item.title} year={item.year} title={item.title} sub={item.sub} />
            ))}
          </div>
        </ScrollReveal>
      </div>
